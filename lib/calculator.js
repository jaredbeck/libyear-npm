...
var calculatorEvents = new events.EventEmitter();
...
function assertPackagesAreInstalled(packages) {
  _.forEach(packages, function(value, key) {
    if (_.isUndefined(value['current'])) {
      var e = new Error(
        'Unable to determine current version of package: ' +
          key +
          '\n' +
...
function calculator(outdated, releaseTime) {
  ...
  _.forEach(packages, function(value, key) {
    ...
    calculatorEvents.emit('libScanned', result[key]);
  });
...
module.exports = { calculator, calculatorEvents };
