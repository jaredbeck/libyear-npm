  ...
  try {
    ...
    if (directory) {
      if (!fs.existsSync(directory))
        throw Error('Path provided does not exists: ' + directory);
      if (!fs.lstatSync(directory).isDirectory())
        throw Error('Path provided is not a directory: ' + directory);
    ...
    if (
      error.status === 1 &&
      (_.isNull(error.error) || _.isUndefined(error.error))
    ) {
      stdout = error.stdout;
    } else {
